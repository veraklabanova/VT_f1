# Vlastnim tempem — Product Architecture Blueprint (PAB v2)
...
- **Verze:** v2
- **Datum:** 2026-04-04
- **Vstupni dokumenty:** PA_Vlastnim_tempem_v1.md, PAD_Vlastnim_tempem_v1.md, BACKLOG.md (BL-001 az BL-008)
- **Status:** IMPLEMENTOVANO (MVP)
- **Predchozi verze:** PAB_Vlastnim_tempem_v1.md
...
| **Primarni: Osoba s kognitivnim postizenim** | Muze web pouzivat samostatne (lehci postizeni). Zjednoduseny rezim UI. |
...
| 12 | Zjednoduseny rezim — CSS prepinac (vetsi fonty, mene barev, vetsi klikaci plochy) | P1 |
...
flowchart TD
    ...
    B -->|Mam potize s pameti| C[Onboarding — role: osoba_s_postizenim]
    B -->|Staram se o blizkeho| D[Onboarding — role: pecujici]
...
| B2 | Neporozumeni formulari | Stredni | Vysoky | Testovani s pilotni skupinou, napovedy |
...
### Principy zjednodusenho rezimu
Aktivuje se automaticky pro role „osoba s postizenim". Implementace: CSS trida `simplified-mode`.
- Vetsi fonty, mene barev, vetsi klikaci plochy (min. 48x48px)
- Stejny layout jako standardni rezim
- Jednodussi formulace textu

### Formular miry postizeni — dve zneni

**Varianta A — Osoba s postizenim (1. osoba):**

Uvodni text: „Je dobre vyplnit to s nekym blizkym."
...
| # | Obrazovka | Rezim |
|---|---|---|
| 1 | Landing page | Univerzalni — 3 karty (osoba s postizenim / pecujici / organizace), tlacitka "Zacit" vedou na onboarding [ZMENA v2] |
| 2 | Onboarding pruvodce | Vicekrokovy wizard: role → dotaznik → tema → PDF stazeni [ZMENA v2: nova obrazovka, viz BL-001] |
...
| 6 | Dashboard — Osoba s postizenim | Zjednoduseny (CSS) — velke karty, minimalni prvky |
...
| 9 | Formular miry postizeni | 2 zneni x 2 vizualni rezimy |
| 10 | Vyber tematu | Filtrovano dle I8 — nedostupna temata oznacena "Pripravujeme" [ZMENA v2: viz BL-008] |
| 11 | Generovani a stazeni | Standardni + zjednoduseny |
...
| D38 | Zjednoduseny rezim = CSS prepinac (vetsi fonty, mene barev) |
...
## PRILOHA: PREHLED ZMEN v2 vs v1

| BL ID | Nazev zmeny | Dopad |
|---|---|---|
| BL-001 | Onboarding flow bez registrace | Novy onboarding pruvodce, I5 relaxovan, freemium bez uctu |
| BL-002 | Archiv cviceni — nova admin sekce | 7 funkcnich oblasti admina, stavovy prechod Archivovany → Ke schvaleni |
| BL-003 | 3 akce ve fronte revize | Schvalit / Archivovat / Zamitnout (puvodne 2 akce) |
| BL-004 | Placeholder obrazky pro demo cviceni | Implementacni detail — zadny dopad na PAB |
| BL-005 | Admin demo mod bez autentizace | Vyvojovy nastroj — zadny dopad na PAB |
| BL-006 | Dva paralelni registracni flow | Prima registrace + po onboardingu, prenos dat z localStorage |
| BL-007 | Admin odhlasovaci tlacitko | UX doplneni admin navigace |
| BL-008 | Filtrovani temat v onboardingu dle I8 | Nedostupna temata zobrazuji "Pripravujeme" |
