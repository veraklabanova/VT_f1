# UAT — Vlastním tempem (MVP + UX Redesign)

**Projekt:** Vlastním tempem
**Organizace:** At Your Own Pace, z.s.
**Verze dokumentu:** v3.0
**Datum:** 2026-04-04
**Vstupní dokumenty:** PAB_Vlastnim_tempem_v2.md (PAB v2), PAB_Vlastnim_tempem_v3.md (PAB v3), BACKLOG.md (BL-001 až BL-011)
**Doména:** HealthTech / SocialTech — kognitivní rehabilitace, neziskový sektor
**Formát testů:** Klasický (Krok — Akce — Očekávaný výsledek)

> **Změny oproti v2:** Testy aktualizovány dle backlogu BL-009 až BL-011 (UX Redesign).
> Nové a změněné testy jsou označeny **[NOVÉ v3]** resp. **[ZMĚNA v3]**.
> Testy z v2 (BL-001 až BL-008) ponechány s původním označením **[NOVÉ v2]** / **[ZMĚNA v2]**.
...
|------|------|--------------------|
| 1 | Otevřete landing page | Zobrazí se 3 karty: „Chci trénovat svou paměť" / „Hledám sešit pro někoho blízkého" / „Jsme organizace". V hlavičce je tlačítko „Přihlásit se" [ZMĚNA v3: nové texty karet, viz BL-011] |
...
| 4 | Odešlete formulář | Systém vytvoří účet typu „jednotlivec" s rolí „osoba_s_postížením". Odeslán ověřovací email |
...
| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Na landing page klikněte na kartu „Hledám sešit pro někoho blízkého" (tlačítko „Začít") [ZMĚNA v3: nový text karty, viz BL-011] | Spustí se onboarding průvodce na `/onboarding?role=pecujici` (bez registrace) |
...
| 3 | Po stažení sešitu klikněte na nabídku registrace „Uložit výsledek a vytvořit bezplatný účet" [ZMĚNA v3: nový text, viz BL-011] | Otevře se registrační formulář s předvyplněnou rolí „pečující" |
...
#### TC-2.02: Přihlášení osoby s postižením — automatický zjednodušený režim + a11y-theme (High) [ZMĚNA v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Přihlaste se s účtem role „osoba_s_postížením" | Systém přihlásí uživatele |
| 2 | Zkontrolujte zobrazení dashboardu | Dashboard se automaticky zobrazí ve zjednodušeném režimu (CSS třída `simplified-mode`) a s aktivním a11y tématem (CSS třída `.a11y-theme`): větší fonty (+2–4px oproti standardu), vyšší kontrastní rámečky, větší klikací plochy (min. 48×48 px), velké karty, minimální prvky [ZMĚNA v3: přidána kontrola .a11y-theme, viz BL-009] |
...
#### TC-3.01: Vyplnění formuláře — role Pečující (varianta B, 3. osoba) (High) [ZMĚNA v3]
...
| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Na dashboardu osoby s postižením klikněte na výzvu k formuláři | Otevře se formulář ve variantě A (znění v 1. osobě), ve zjednodušeném vizuálním režimu s aktivním a11y tématem [ZMĚNA v3: a11y-theme] |
| 2 | Ověřte úvodní text | Zobrazí se: „Je dobré vyplnit to s někým blízkým." + uklidňující podnadpis [ZMĚNA v3: přidán uklidňující text, viz BL-011] |
| 3 | Ověřte nadpis dotazníku | Nadpis: „Pojďme zjistit, co vám bude nejlépe vyhovovat" [ZMĚNA v3: nový nadpis, viz BL-011] |
| 4 | Zodpovězte všech 7 otázek | Znění otázek je v 1. osobě, konverzačním tónu (ne klinickém). Vizuální styl zjednodušený. Odpovědi formou blokových karet [ZMĚNA v3: empatický tón + blokové karty, viz BL-009/BL-011] |
| 5 | Odešlete formulář | Průměr vyhodnocen, míra postižení uložena v profilu |
...
#### TC-3.08: Formulář — progress bar a navigace (Medium) [NOVÉ v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Otevřete formulář v onboarding průvodci | Zobrazí se progress bar s badgem „Krok X z Y" indikující aktuální pozici v dotazníku [NOVÉ v3: viz BL-009] |
| 2 | Odpovězte na první otázku a přejděte na další | Progress bar se aktualizuje. Zobrazí se zvětšené tlačítko „Zpět" (outline button, ne malý textový odkaz) [NOVÉ v3: viz BL-009] |
| 3 | Klikněte na tlačítko „Zpět" | Systém přejde na předchozí otázku. Předchozí odpověď je zachována |

---

### Modul 4 — Výběr tématu a generování sešitu (jednotlivec)

#### TC-4.01: Výběr tématu a stažení prvního sešitu zdarma (High) [ZMĚNA v3]
...
#### TC-4.07: Nedostupné téma — nedostatek úkolů (Medium) [ZMĚNA v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Přejděte na výběr tématu | Témata, která nemají min. 10 schválených úkolů s min. 4 různými kognitivními tagy pro příslušnou obtížnost, jsou zobrazena s opacity 0.4, bez falešných tlačítek, pouze s textem kurzívou „Připravujeme" (I8) [ZMĚNA v3: nový vizuální stav, viz BL-009] |
...
### Modul 8 — Zjednodušený režim UI a A11y téma

#### TC-8.01: Automatická aktivace zjednodušeného režimu + a11y-theme (High) [ZMĚNA v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Přihlaste se jako „osoba_s_postížením" | Systém automaticky aktivuje CSS třídu `simplified-mode` a CSS třídu `.a11y-theme` [ZMĚNA v3: přidána .a11y-theme, viz BL-009] |
| 2 | Zkontrolujte vizuální styl | Větší fonty (+2–4px oproti standardu), vyšší kontrastní rámečky, větší klikací plochy (min. 48×48 px), velké karty, minimální prvky [ZMĚNA v3: upřesněno, viz BL-009] |

#### TC-8.02: Standardní režim pro pečující a organizaci (Medium)

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Přihlaste se jako „pečující" | Dashboard ve standardním režimu (bez .a11y-theme) |
| 2 | Přihlaste se jako „organizace" | Dashboard ve standardním režimu (bez .a11y-theme) |

#### TC-8.03: Zjednodušený režim — formulář (Medium) [ZMĚNA v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Jako osoba s postižením otevřete formulář míry postižení | Formulář je zobrazen ve zjednodušeném vizuálním režimu se zněním v 1. osobě. Odpovědi jsou formou klikatelných blokových karet. A11y-theme je aktivní [ZMĚNA v3: blokové karty + a11y-theme, viz BL-009] |

#### TC-8.04: A11y-theme se aktivuje v onboarding wizardu (High) [NOVÉ v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Na landing page klikněte na kartu „Chci trénovat svou paměť" | Spustí se onboarding průvodce s rolí „osoba_s_postížením" |
| 2 | Zkontrolujte vizuální styl onboarding průvodce | CSS třída `.a11y-theme` je aktivní — fonty zvětšeny o 2–4px, vyšší kontrastní rámečky [NOVÉ v3: viz BL-009] |
| 3 | Projděte dotazníkem | Dotazník zobrazuje blokové karty odpovědí s a11y úpravami |
...
| 2 | Zkontrolujte obsah dashboardu | Dashboard NEOBSAHUJE kartu/sekci dotazníku (assessment). Dotazník se zobrazuje pouze uživatelským rolím (osoba_s_postížením, pečující), nikoli admin roli |
...
#### TC-15.01: Landing page — vstupní cesty (High) [ZMĚNA v3]
...
#### TC-15.02: Landing page — vizuální design (Medium) [NOVÉ v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Otevřete landing page | Pozadí je teplá lomená bílá (#FEFDFB), ne čistě bílá. Hlavní text je antracitový (#2C2A29), ne čistě černý [NOVÉ v3: barevná paleta, viz BL-010] |
| 2 | Zkontrolujte font | Hlavní font je Nunito (zaoblený sans-serif z Google Fonts). Žádná patková (serif) písma v nadpisech [NOVÉ v3: viz BL-010] |
| 3 | Zkontrolujte karty | Karty mají border-radius 16px, jemný rozptýlený stín (box-shadow), žádné tvrdé rámečky [NOVÉ v3: měkká geometrie, viz BL-010] |
| 4 | Zkontrolujte tlačítka | Tlačítka mají tvar pilulky (border-radius: 999px). Primární barva je šalvějová zelená (#5BA48A) [NOVÉ v3: viz BL-010] |
| 5 | Zkontrolujte sekci „Jak to funguje" | Kroky jsou popsány přátelským jazykem (ne technickým) [NOVÉ v3: viz BL-011] |

---

### Modul 16 — Onboarding průvodce (bez registrace) [NOVÉ v2]

#### TC-16.01: Onboarding — kompletní flow pro pečující (High) [ZMĚNA v3]
...
| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Na landing page klikněte na kartu „Chci trénovat svou paměť" [ZMĚNA v3: nový text karty] | Spustí se onboarding průvodce. Krok 1: Výběr role (předvyplněna role „osoba_s_postížením"). Aktivní CSS třída `.a11y-theme` [ZMĚNA v3: a11y-theme, viz BL-009] |
| 2 | Potvrďte roli a přejděte na krok 2 | Zobrazí se dotazník ve variantě A (1. osoba), zjednodušený vizuální styl + a11y-theme (fonty +2–4px), 7 otázek. Blokové karty odpovědí [ZMĚNA v3: viz BL-009] |
| 3 | Zodpovězte všech 7 otázek a odešlete | Průměr vyhodnocen, míra postížení určena |
| 4 | Přejděte na krok 3 — výběr tématu | Dostupná témata zvýrazněna, nedostupná: opacity 0.4 + kurzíva „Připravujeme" [ZMĚNA v3: viz BL-009] |
| 5 | Vyberte téma, počkejte na generování, stáhněte sešit | Sešit stažen zdarma bez registrace. Fallback odkaz pod CTA tlačítkem [NOVÉ v3: viz BL-009] |
...
| 3 | Zobrazí se přímo výběr tématu | Dostupná témata zvýrazněna, nedostupná: opacity 0.4 + kurzíva „Připravujeme" [ZMĚNA v3: viz BL-009] |
...
#### TC-16.04: Onboarding — výběr tématu zobrazuje dostupnost (High) [ZMĚNA v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | V onboarding průvodci přejděte na krok výběru tématu | Zobrazí se všechna témata (např. Rodina, Zahrada, Dům, Jaro, Domácí práce). Nadpis: „O čem si chcete číst a přemýšlet?" [ZMĚNA v3: viz BL-011] |
| 2 | Zkontrolujte dostupná témata (splňující I8) | Dostupná témata mají stín, border-radius 16px a tlačítko „Vybrat". Jsou klikatelná [ZMĚNA v3: stín místo zeleného rámečku, viz BL-010] |
| 3 | Zkontrolujte nedostupná témata (nesplňující I8) | Nedostupná témata mají opacity 0.4, text kurzívou „Připravujeme". Žádná falešná tlačítka. NEJSOU klikatelná [ZMĚNA v3: redesign, viz BL-009] |
...
### Modul 19 — Vizuální design a emocionální design [NOVÉ v3]

#### TC-19.01: Barevná paleta — teplá lomená bílá (Medium) [NOVÉ v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Otevřete libovolnou stránku aplikace | Pozadí stránky je teplá lomená bílá (#FEFDFB), ne čistě bílá (#FFFFFF) [NOVÉ v3: viz BL-010] |
| 2 | Zkontrolujte barvu hlavního textu | Text je antracitový (#2C2A29), ne čistě černý (#000000) [NOVÉ v3: viz BL-010] |
| 3 | Zkontrolujte primární akcentovou barvu | Tlačítka a ikonové prvky používají šalvějovou zelenou (#5BA48A) [NOVÉ v3: viz BL-010] |

#### TC-19.02: Typografie — font Nunito (Medium) [NOVÉ v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Otevřete libovolnou stránku a zkontrolujte font nadpisů | Nadpisy používají font Nunito (zaoblený sans-serif). Žádná patková (serif) písma [NOVÉ v3: viz BL-010] |
| 2 | Zkontrolujte font běžného textu | Běžný text rovněž používá Nunito [NOVÉ v3: viz BL-010] |

#### TC-19.03: Měkká geometrie — karty a tlačítka (Medium) [NOVÉ v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Zkontrolujte karty na landing page | Karty mají border-radius 16px a jemný rozptýlený stín (box-shadow: 0 10px 25px rgba(0,0,0,0.05)). Žádné tvrdé rámečky [NOVÉ v3: viz BL-010] |
| 2 | Zkontrolujte CTA tlačítka | Tlačítka mají tvar pilulky (border-radius: 999px) [NOVÉ v3: viz BL-010] |

#### TC-19.04: Štítek „Zdarma" — vizuální styl (Low) [NOVÉ v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Přejděte na stránku stažení sešitu | Štítek „Zdarma" je zobrazen jako světle zelený textový tag, ne jako černá badge. Nekonkuruje vizuálně CTA tlačítku [NOVÉ v3: viz BL-009] |

#### TC-19.05: Fallback odkaz pro stažení (Medium) [NOVÉ v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Přejděte na stránku stažení sešitu | Pod hlavním CTA tlačítkem „Stáhnout můj první sešit (zdarma)" je zobrazen fallback odkaz: „Pokud stahování nezačalo, klikněte zde" [NOVÉ v3: viz BL-009] |
| 2 | Klikněte na fallback odkaz | Sešit se stáhne (alternativní metoda stažení pro případ blokace vyskakovacích oken) |

---

### Modul 20 — Tone of Voice a copywriting [NOVÉ v3]

#### TC-20.01: Landing page — empatický tón (High) [NOVÉ v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Otevřete landing page | H1: „Trénujte paměť a myšlení. V klidu a vlastním tempem." (NE „Kognitivní trénink vlastním tempem") [NOVÉ v3: viz BL-011] |
| 2 | Zkontrolujte podnadpis | Podnadpis NEOBSAHUJE termín „kognitivní postížení". Používá empatický, přátelský jazyk [NOVÉ v3: viz BL-011] |
| 3 | Zkontrolujte texty karet | Karta 1: „Chci trénovat svou paměť" (NE „Mám potíže s pamětí"). Karta 2: „Hledám sešit pro někoho blízkého" (NE „Starám se o blízkého") [NOVÉ v3: viz BL-011] |

#### TC-20.02: Dotazník — empatický tón (High) [NOVÉ v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Otevřete dotazník v onboarding průvodci | Nadpis: „Pojďme zjistit, co vám bude nejlépe vyhovovat" (NE „Krátký dotazník") [NOVÉ v3: viz BL-011] |
| 2 | Zkontrolujte podnadpis | Obsahuje uklidňující text (např. „Nemusíte se ničeho obávat...") [NOVÉ v3: viz BL-011] |
| 3 | Zkontrolujte znění otázek | Otázky jsou formulovány konverzačně, jako běžný rozhovor — ne klinicky, ne jako lékařský test [NOVÉ v3: viz BL-011] |

#### TC-20.03: Výběr tématu — empatický tón (Medium) [NOVÉ v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Přejděte na výběr tématu | Nadpis: „O čem si chcete číst a přemýšlet?" (NE „Vyberte téma") [NOVÉ v3: viz BL-011] |
| 2 | Zkontrolujte banner míry obtížnosti | Text: „Výborně, máme to! Podle vašich odpovědí jsme pro vás připravili cvičení s obtížností: [míra]." (NE „Určená obtížnost: [míra]") [NOVÉ v3: viz BL-011] |

#### TC-20.04: Stažení sešitu — empatický tón (High) [NOVÉ v3]

| Krok | Akce | Očekávaný výsledek |
|------|------|--------------------|
| 1 | Přejděte na stránku stažení sešitu | Text před stažením: „Skvělé, váš sešit je připraven!" (NE „Váš sešit je připraven") [NOVÉ v3: viz BL-011] |
| 2 | Zkontrolujte text CTA tlačítka | Tlačítko: „Stáhnout můj první sešit (zdarma)" (NE „Stáhnout sešit zdarma") [NOVÉ v3: viz BL-011] |
| 3 | Stáhněte sešit a zkontrolujte text po stažení | Text po stažení: „Sešit se stahuje k vám do počítače." (NE „Sešit byl stažen!") [NOVÉ v3: viz BL-011] |
| 4 | Zkontrolujte text registrační nabídky | Text: „Uložit výsledek a vytvořit bezplatný účet" (NE „Vytvořit účet pro další sešity") [NOVÉ v3: viz BL-011] |
...
| ID | Kritérium |
|----|-----------|
| AK-B01 | Systém umožňuje registraci a přihlášení pro 3 typy uživatelů: osoba s postížením, pečující osoba, organizace |
| AK-B02 | Každý registrovaný uživatel obdrží ověřovací email a bez jeho potvrzení se nemůže přihlásit |
| AK-B03 | Formulář míry postížení pokrývá 5 dimenzí (paměť, orientace, pozornost, jazyk, samostatnost) prostřednictvím 7 otázek |
| AK-B04 | Formulář existuje ve dvou zněních: 1. osoba (pro osobu s postížením) a 3. osoba (pro pečující) |
| AK-B05 | Systém korektně vyhodnocuje míru postížení (lehká/střední/těžší) na základě průměru odpovědí se správnými prahovými hodnotami (1.6 / 1.7 a 2.3 / 2.4) |
| AK-B06 | Jednotlivec obdrží 1 PDF sešit (12 stran: titulní + 10 úkolů + instrukce) odpovídající jeho míře postížení a zvolenému tématu |
...
| AK-B20 | Aplikace používá empatický tone of voice bez odborného žargonu — žádné „kognitivní postížení" v uživatelském rozhraní (BL-011) [NOVÉ v3] |
| AK-B21 | Vizuální design vytváří teplý, přátelský dojem: font Nunito, barvy #FEFDFB/#2C2A29/#5BA48A, zaoblené tvary (BL-010) [NOVÉ v3] |

### System-level akceptační kritéria

| ID | Kritérium |
|----|-----------|
| AK-S01 | Zjednodušený režim (CSS třída `simplified-mode`) a a11y téma (CSS třída `.a11y-theme`) se automaticky aktivují pro roli „osoba_s_postížením": větší fonty (+2–4px), vyšší kontrastní rámečky, klikací plochy min. 48×48 px [ZMĚNA v3: přidán .a11y-theme, viz BL-009] |
| AK-S02 | Organizace neprochází formulářem míry postížení — vždy dostává 3 sešity (I7) |
...
| AK-S07 | Míra postížení musí být vyhodnocena před sestavením sešitu pro jednotlivce (I2) |
...
| AK-S12 | Formulář míry postížení je editovatelný — poslední stav přepisuje předchozí (A13) |
...
| AK-S26 | Pozadí aplikace: #FEFDFB (teplá lomená bílá), text: #2C2A29 (antracitová), primární: #5BA48A (šalvějová zelená) (BL-010) [NOVÉ v3] |
| AK-S27 | Font Nunito (Google Fonts) jako hlavní font; žádná patková písma (BL-010) [NOVÉ v3] |
...
| 15 | **CSS přesnost barevné palety a geometrie** | Přesná shoda CSS hodnot (#FEFDFB, #2C2A29, #5BA48A, border-radius, box-shadow) s designovým zadáním vyžaduje vizuální regresní testování. | Vizuální regresní testy (Percy, Chromatic), CSS audit nástrojů [NOVÉ v3] |
| 16 | **Google Fonts načítání (Nunito)** | Spolehlivost načtení fontu Nunito z Google Fonts závisí na CDN dostupnosti a síťové konektivitě. | Performance testy, fallback font ověření, offline testování [NOVÉ v3] |
